...
load = 0
loadfile = 'signal_1500_50W.h5'
savefile = 'signal_1500_50W.h5'
dataset = 'Signal_1500_50W.mat'

# input image dimensions
# number of convolutional filters to use

def Net_model(lr=0.01,decay=1e-6,momentum=0.3):
    model = Sequential()
    model.add(Convolution2D(25, 1, 9,
    ...
    model.add(Convolution2D(15, 1, 9,subsample=(1, 2)))
    model.add(Activation(ELU(alpha=1.0)))
    model.add(MaxPooling2D(pool_size=(1, 3),strides = (1,2)))
    ...
    model.add(Dense(120,W_regularizer=l2(0.001))) #Full connection
    model.add(Dropout(0.0))
    model.add(Activation(ELU(alpha=1.0)))
    model.add(BN(epsilon=1e-06, mode=0, axis=1, momentum=momentum))
    
    model.add(Dense(40,W_regularizer=l2(0.001))) #Full connection
    model.add(Dropout(0.0))
    model.add(Activation(ELU(alpha=1.0)))
    model.add(BN(epsilon=1e-06, mode=0, axis=1, momentum=momentum))
